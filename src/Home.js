...
const features = [
  {
    title: "Code Hints",
    description:
      "Get intelligent hints as you write code to improve your learning experience.",
  },
  {
    title: "Code Editor Suggestions",
    description:
      "Receive suggestions in a smart code editor to enhance your coding skills.",
  },
  {
    title: "Online Exams",
    description:
      "Take coding challenges and assessments online with real-time feedback.",
  },
  {
    title: "Graphical Representation",
    description:
      "Visualize algorithms and code execution with interactive graphs.",
  },
];
...
const testimonials = [
  {
    name: "Alice",
    feedback:
      "This tool transformed my coding practice and made learning so much easier!",
  },
  {
    name: "Bob",
    feedback:
      "The hints and suggestions are incredibly helpful. Highly recommend!",
  },
];
...
function Home({ username, userId }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 px-4">
          {features.map((feature, index) => (
            <div
              key={index}
              className="p-6 border border-gray-300 rounded-lg shadow-lg bg-white transition-transform duration-300 hover:shadow-xl hover:scale-105"
            >
      ...
      {/* Interactive Demo */}
      <section
        className={`py-10 text-center transition-opacity duration-700 ${
          visible ? "opacity-100" : "opacity-0"
        }`}
      >
      ...
      {/* Testimonials */}
      <section
        className={`py-10 bg-gray-50 transition-opacity duration-700 ${
          visible ? "opacity-100" : "opacity-0"
        }`}
      >
        ...
        <div className="max-w-6xl mx-auto flex flex-col space-y-4 px-4">
          {testimonials.map((testimonial, index) => (
            <blockquote
              key={index}
              className="border-l-4 border-blue-500 italic bg-white p-6 shadow-md transition-shadow duration-300 hover:shadow-xl hover:bg-gray-100"
            >
              <p className="mb-2 text-lg">"{testimonial.feedback}"</p>
              <cite className="block text-right text-gray-600">
                - {testimonial.name}
              </cite>
            </blockquote>
          ))}
